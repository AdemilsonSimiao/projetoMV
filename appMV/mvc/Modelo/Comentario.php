...
class Comentario extends Modelo {
    const BUSCAR_COMENTARIOS= 'SELECT idcomentario, comentario, idatualizacao, idusuario, nome FROM comentarios JOIN usuarios USING (idusuario) WHERE idatualizacao = ?';
    const INSERIR_COMENTARIO= 'INSERT INTO comentarios (comentario, idatualizacao, idusuario) VALUES (?, ?, ?)';
    const EDITAR_COMENTARIO= 'UPDATE comentarios SET comentario = ? WHERE idatualizacao = ? && idcomentario = ?';
    const DELETAR_COMENTARIO = 'DELETE FROM comentarios WHERE idcomentario = ?';
